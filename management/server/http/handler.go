...
func NewAPIHandler(
	...
	// default chained middlewares
	// @todo should metricsMiddleware be part of the chain?
	chainedMiddlewares := func(next http.Handler) http.Handler {
		return corsMiddleware.Handler(authMiddleware.Handler(acMiddleware.Handler(next)))
	}

	router.Use(metricsMiddleware.Handler, chainedMiddlewares)

	if _, err := integrations.RegisterHandlers(ctx, prefix, router, accountManager, chainedMiddlewares, integratedValidator, appMetrics.GetMeter()); err != nil {
